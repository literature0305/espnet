...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ] && ! [[ " ${skip_stages} " =~ [[:space:]]3[[:space:]] ]]; then
    ...
    if [ "${feats_type}" = raw ]; then
        ...
        for dset in ${_dsets}; do
            ...
            if [ "${dset}" = "${train_set}" ] && [ -f "data/${dset}/utt2weight" ]; then
                cp "data/${dset}/utt2weight" "${data_feats}${_suf}/${dset}/utt2weight"
            fi
...
if [ ${stage} -le 10 ] && [ ${stop_stage} -ge 10 ] && ! [[ " ${skip_stages} " =~ [[:space:]]10[[:space:]] ]]; then
    ...
    if [ -f "${_asr_train_dir}/utt2weight" ]; then
        _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/utt2weight,utt2weight,text_float "
    fi
    if ${use_prompt}; then
